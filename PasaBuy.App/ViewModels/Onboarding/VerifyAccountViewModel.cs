    ...
    public class VerifyAccountViewModel : BaseViewModel
    {
        ...
        private void SfButton_Clicked(object obj)
        {
            ...
            catch (Exception err)
            {
                ...
                {
                    new Controllers.Notice.Alert("Error Code: DVV1URS-AC1VAVM", err.ToString(), "OK");
                    Microsoft.AppCenter.Analytics.Analytics.TrackEvent("DEV-DVV1URS-AC1VAVM-" + err.ToString());
                }
                else
                {
                    new Controllers.Notice.Alert("Something went Wrong", "Please contact administrator. Error Code: DVV1URS-AC1VAVM.", "OK");
                    Microsoft.AppCenter.Analytics.Analytics.TrackEvent("LIVE-DVV1URS-AC1VAVM-" + err.ToString());
                }
            }
        }
