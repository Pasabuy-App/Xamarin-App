...
namespace PasaBuy.App.ViewModels.Chat
{
    ...
    public class StoreConversationViewModel : BaseViewModel
    {
        ...
        {
            if (msg.s == PSACache.Instance.UserInfo.stid)
            {
        ...
        public void LoadMessage(string offset)
        {
            try
            {
                Http.SocioPress.Message.Instance.GetByRecepient(user_id, PSACache.Instance.UserInfo.stid, offset, "store", odid, (bool success, string data) =>
                {
                    if (success)
                    {
                        ...
                        {
                            bool isreceived = chat.data[i].sender != PSACache.Instance.UserInfo.stid ? true : false;
            ...
            catch (Exception err)
            {
                ...
                {
                    new Controllers.Notice.Alert("Error Code: SPV1MSG-GBR1SCVM", err.ToString(), "OK");
                    Microsoft.AppCenter.Analytics.Analytics.TrackEvent("DEV-SPV1MSG-GBR1SCVM-" + err.ToString());
                }
                else
                {
                    new Controllers.Notice.Alert("Something went Wrong", "Please contact administrator. Error Code: SPV1MSG-GBR1SCVM.", "OK");
                    Microsoft.AppCenter.Analytics.Analytics.TrackEvent("LIVE-SPV1MSG-GBR1SCVM-" + err.ToString());
                }
            }
        }
            ...
            if (!string.IsNullOrWhiteSpace(this.NewMessage))
            {
                ...
                try
                {
                    if (count == 0)
                    {
                        ...
                        Http.SocioPress.Message.Instance.Insert(this.NewMessage, user_id, PSACache.Instance.UserInfo.stid, "store", odid, (bool success, string data) =>
                        {
                            if (success)
                            {
                ...
                catch (Exception err)
                {
                    if (PSAConfig.isDebuggable)
                    {
                        new Controllers.Notice.Alert("Error Code: SPV1MSG-I1SCVM", err.ToString(), "OK");
                        Microsoft.AppCenter.Analytics.Analytics.TrackEvent("DEV-SPV1MSG-I1SCVM-" + err.ToString());
                    }
                    else
                    {
                        new Controllers.Notice.Alert("Something went Wrong", "Please contact administrator. Error Code: SPV1MSG-I1SCVM.", "OK");
                        Microsoft.AppCenter.Analytics.Analytics.TrackEvent("LIVE-SPV1MSG-I1SCVM-" + err.ToString());
                    }
                }
            }
        }
